...
public class Tester {
    ...
    public static Set<Student> findDuplicates(List<Student> students) {
        ...
        Set<String> danceNominations = new HashSet<>();
        Set<String> musicNominations = new HashSet<>();

        for (Student student : students) {
            if (student.getEvent().equalsIgnoreCase("Dance")) {
                if (!danceNominations.add(student.getEmailId())) {
                    duplicateStudents.add(student);
                }
            } else if (student.getEvent().equalsIgnoreCase("Music")) {
                if (!musicNominations.add(student.getEmailId())) {
                    duplicateStudents.add(student);
                }
            }
        }
